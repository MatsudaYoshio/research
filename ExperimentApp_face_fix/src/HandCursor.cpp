...
HandCursor::HandCursor():cursor_colors(this->initial_cursor_color_num), cursor_color_state(this->initial_cursor_color_num) {
	...
	random_device rd;
	mt19937 mt(rd());
	copy_n(begin(this->initial_cursor_colors), this->initial_cursor_color_num, begin(this->cursor_colors));

	shuffle(begin(this->cursor_colors), end(this->cursor_colors), mt); // 乱数でシャッフル
...
void HandCursor::update() {
	...
	this->personal_id2user_id.resize(people_num);
	fill_n(begin(this->personal_id2user_id), people_num, NOT_USER);
	for (const auto& ud : this->user_data) {
		this->personal_id2user_id[ud.second.personal_id] = ud.first;
	}
...
void HandCursor::renew_user_data(const int personal_id, const double face_size, const long long int user_id) {
	this->user_data[user_id].personal_id = personal_id;
	...
	if (this->user_data[user_id].state == STATE::INACTIVE) { // カーソルが消えている状態から復帰したら
		...
		this->transform_point(this->user_data[user_id].face_point, this->user_data[user_id].first_face_point);
	...
	if (this->user_data[user_id].face_point.x > CAMERA_W / 2) {
		this->left_user_cursor_log.emplace_back(this->user_data[user_id].cursor_point);
	}
	else {
		this->right_user_cursor_log.emplace_back(this->user_data[user_id].cursor_point);
	}
}
