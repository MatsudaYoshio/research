...
void SimulatedAnnealing::setup(HandCursor* hc, vector<long long int>* active_window_list, vector<long long int>* main_window_user_list, unordered_map<long long int, SubWindow>* sub_windows) {
	this->hc = hc;
	...
	for (const auto& s : this->next_state) {
		try {
			for (const auto& id : *this->main_window_user_list) {
				...
				if (s.second.intersects(ofRectangle(ofClamp(this->hc->user_data.at(id).transformed_cursor_point.x() - HALF_MAX_SUB_WINDOW_W, 0, DISPLAY_W), ofClamp(this->hc->user_data.at(id).transformed_cursor_point.y() - HALF_MAX_SUB_WINDOW_H, 0, DISPLAY_H), ofClamp(MAX_SUB_WINDOW_W, 0, DISPLAY_W), ofClamp(MAX_SUB_WINDOW_H, 0, DISPLAY_H)))) {
