...
private:
	...
	static constexpr double fitness_multiple{ 0.016 * population_size + 0.4 }; // スケーリング時のパラメータ(最良の個体が次世代に残す個体数の期待値を表し、集団数が50～100ならば1.2～2.0ぐらいがいい)
	...
	array<genome_type, population_size> population; // 集団
	array<genome_type, population_size> population_new; // 選択淘汰によって新たにできる集団
	array<double, population_size> fitness; // 適応度
