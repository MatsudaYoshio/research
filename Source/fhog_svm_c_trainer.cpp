...
int main()
{
	const string path = "D:/machine_learning_data/hand/20170813/"; // csvファイルがあるディレクトリパス

	using X_type = matrix<double, 1984, 1>; // 次元数をそのまま記述、100*100pixelの画像に対するfhog特徴量の次元は3752, 80x80は8*8*31で1984
	using kernel_type = radial_basis_kernel<X_type>; // カーネルの型
	//using kernel_type = histogram_intersection_kernel<X_type>;
	std::vector<X_type> X;
	std::vector<double> y;
	...
	*/
	...
	serialize(path + "c_svm_function.dat") << learned_function; // 訓練結果をファイルに出力
