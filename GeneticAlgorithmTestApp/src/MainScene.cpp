		...
		case static_cast<int>(Icon::STATE::POINT) :
			for (auto u = begin(this->user_id_list); u != end(this->user_id_list); ++u) {
				try {
					...
					if (i.is_inside(ofPoint(DISPLAY_W - this->hc->track_data.at(*u).current_pointer.x, this->hc->track_data.at(*u).current_pointer.y))) {
...
void MainScene::draw() {
	...
	ofRectRounded(0.52*DISPLAY_W, 0.093*DISPLAY_H, 0.026*DISPLAY_W, 0.815*DISPLAY_H, 20);
	...
	try {
		for (auto &id : this->user_id_list) {
			...
			for (int i = 0; i < 100; ++i) {
				...
				ofCircle(CAMERA_W - this->hc->track_data.at(id).current_pointer.x, this->hc->track_data.at(id).current_pointer.y, r);
			}
		}
	}
	catch (std::out_of_range&) {}
