...
void SceneManager::draw_face() const {
	...
	for (int i = 0; i < people_num; ++i) {
		if (this->hc->pose_key_points[NOSE_X(i)] == 0.0 || this->hc->pose_key_points[NOSE_Y(i)] == 0.0 || this->hc->personal_id2user_id[i] != NOT_USER) {
			continue;
		}
...
void SceneManager::draw_cursor() const {
	for (const auto& ud : this->hc->user_data) {
		...
		switch (ud.second.hand) {
		case HandCursor::USING_HAND::RIGHT:
			...
			break;
		case HandCursor::USING_HAND::LEFT:
